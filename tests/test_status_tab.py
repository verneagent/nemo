...
from nemo.status_tab import _tab_name, _find_nemo_tab, update_status, remove_tab
...
def test_find_nemo_tab_found():
  tabs = [
    {"tab_id": "t1", "tab_name": "Messages"},
    {"tab_id": "t2", "tab_name": "🟢 Nemo (claude-opus-4-6)"},
  ]
  assert _find_nemo_tab(tabs)["tab_id"] == "t2"


def test_find_nemo_tab_not_found():
  tabs = [{"tab_id": "t1", "tab_name": "Messages"}]
...
def test_update_status_updates_existing():
  tabs = [{"tab_id": "t1", "tab_name": "🟢 Nemo (opus)"}]
